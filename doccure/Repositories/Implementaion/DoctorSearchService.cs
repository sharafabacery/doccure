...
namespace doccure.Repositories.Implementaion
{
	public class DoctorSearchService : IDoctorSearch
	{
		...
		public async Task<List<DoctorSearchReturned>> SearchDoctors(DoctorSearchBarRequest doctorSearchBarRequest)
		{
			...
			var ClinicIds = DoctorSearchReturneds.Select(c => c.ClinicID).ToString();
			FormattableString sql = $"SELECT * FROM ClinicImages WHERE ClinicId IN {ClinicIds}";
			var ClinicImages =  applicationDbContext.ClinicImages.FromSqlInterpolated(sql);
